...
const testOutput = (options: { output: ObjectDiff.Output }) => {
  ...
  const controller = ObjectDiff({ output });
  it("equal", () => {
    const diff = controller.diff({ old: 1, now: 1 });
...
const testIgnore = () => {
  it("ignore=space", () => {
    const controller = ObjectDiff({ ignore: ["white-space"] });
    let old, now;
    old = [" a a a "].join("\n");
    now = ["a a a"].join("\n");
    const diff = controller.diff({ old, now });
  ...
  it("ignore=new-line", () => {
    const controller = ObjectDiff({ ignore: ["new-line"] });
    let old, now;
  ...
  it("ignore=case-insensitive", () => {
    const controller = ObjectDiff({ ignore: ["case-insensitive"] });
    let old, now;
    old = ["abc", "efg"].join("\n");
    now = ["ABC", "EFG"].join("\n");
    const diff = controller.diff({ old, now });
